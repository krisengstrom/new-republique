...
add_action('wp_enqueue_scripts', 'nr_enqueue_styles', 20, 1);
function nr_enqueue_styles() {
...
add_action('wp_enqueue_scripts', 'nr_cdn_resources', 10, 1);		// @see enqueue.php
add_action('wp_enqueue_scripts', 'nr_enqueue_styles', 20, 1);	// @see enqueue.php
add_action('wp_enqueue_scripts', 'nr_enqueue_scripts', 30, 1);	// @see enqueue.php
/**
 * Theme scripts
 *
 * @example wp_enqueue_script( $handle => string, $src => string, $deps => array(), $ver => string, $in_footer => bool );
 */
add_action('wp_enqueue_scripts', 'nr_enqueue_scripts', 30, 1);
function nr_enqueue_scripts() {
	if ( ! is_admin() ) {
...
add_action('wp_enqueue_scripts', 'nr_cdn_resources', 10, 1);
function nr_cdn_resources() {
	...
	if ( ! is_admin() ) {
		 ...
		 */
		if ( NR_GOOGLEFONTS ) {
			wp_enqueue_style( 'nr-google-font', '//fonts.googleapis.com/css?family=Karla', array(), '1.0' );
		}
 ...
 */
add_action('wp_enqueue_scripts', 'nr_google_maps_api');
function nr_google_maps_api() {
	// Update condition
