...
*/
function nr_add_header($headers) {
	$headers['X-UA-Compatible'] = 'IE=edge,chrome=1';
	return $headers;
}
add_filter('wp_headers', 'nr_add_header');
 ...
 */
function nr_revisions( $num ) {
	$num = 5;
    return $num;
}
add_filter('wp_revisions_to_keep', 'nr_revisions', 10 );
 ...
 * Empty search fix
 */
function nr_search_fix_filter($query_vars) {
	if (isset($_GET['s']) and empty($_GET['s'])) {
		$query_vars['s'] = "&#00;"; // No posts should have NUL in them
	}
	return $query_vars;
}
add_filter('request', 'nr_search_fix_filter');
